  ...
  try {
    ...
    if (err.code === '23505') {
      return res.status(409).json({
        status: 'fail',
        message: 'Um post com esse mesmo nome de título já existe!',
      });
    }
    next(err);
  }
...
export const getPostHandler = async (
  ...
  try {
    ...
    if (!post) {
      return next(new AppError(404, 'Não foi encontrado um post com esse número de ID'));
    }
...
export const updatePostHandler = async (
  ...
  try {
    ...
    if (!post) {
      return next(new AppError(404, 'Um post com esse ID não existe'));
    }
